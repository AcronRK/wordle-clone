    ...
    return (
        ...
        <div className='my-[20px] mx-auto max-w-[500px] text-center'>
            {letters.map((letter, index) => {
                let color: string = usedLetters[letter.key] || "keyboard";
                return(
                    <div
                    key={index}
                    className={`m-[5px] w-[40px] h-[50px] inline-block rounded-md uppercase leading-[50px] text-black justify-center bg-${color}`}
                    >
                       {letter.key}
                   </div>
                );
